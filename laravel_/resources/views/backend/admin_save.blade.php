					...
					<div class="span12">
						<div class="widget-box">
							...
							<div class="widget-content nopadding">
								<form id="form-wizard" class="form-horizontal" action="{{URL::asset('admin_saves')}}" method="post" />
									<div id="form-wizard-1" class="step">
										<div class="control-group">
											<label class="control-label">管理员名称</label>
											<div class="controls">
												<input id="admin_name" type="text" name="admin_name" value="{{$data['admin_name']}}" />
											</div>

                                            <label class="control-label">管理员密码：</label>
                                            <div class="controls">
                                                <input id="admin_pwd" type="text" name="admin_pwd" value="{{$data['admin_pwd']}}"  />
                                            </div>
                                            ...
                                            <div class="controls">
                                                @foreach($role_data as $key => $val)
	                                                ...
	                                                <input type="checkbox" name="role_id[]" value="{{$val->id}}" @if($v->role_id == $val->id) checked="checked" @endif /> {{$val->role_name}}
	                                                @endforeach
                                                @endforeach
