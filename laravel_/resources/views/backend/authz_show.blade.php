...
@section('content')
				...
				<div class="widget-box">
					...
					<div class="widget-content nopadding">
						<table class="table table-bordered data-table" align="center">
							...
							<tr class="gradeX">
								<td>{{ $sort -> id }}</td>
								<td>{{ $sort -> authz_name }}</td>
								<td>{{ $sort -> controller }}</td>
								<td>{{ $sort -> action }}</td>
								<td>{{ $sort -> parent_id }}</td>
                                ...
                                @endif
								<td>
								<a class="btn btn-danger" href="{{ URL('authz_save/'.$sort->id.'') }}" role="button">Save</a>
								<a class="btn btn-warning" href="{{ URL('authz_del/'.$sort->id.'') }}" role="button">Delete</a>
								</td>
							</tr>
							@endforeach
        ...
        <script>
            ...
            $('.is_show').blur(function(){
                ...
                /*  目前不知道怎么传 所以 写的绝对路径 */
                $.ajax({
                    url:'http://192.168.1.45/laravel/laravel_/public/authz_gai/'+num+'/'+ids,
                    type:'get',
            ...
            $('button').click(function(){
                var cen = $('#sea').val();
                location.href = 'http://localhost/laravel/laravel_/public/authz_search/'+cen;
            })
