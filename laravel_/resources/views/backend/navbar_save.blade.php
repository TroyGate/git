					...
					<div class="span12">
						<div class="widget-box">
							...
							<div class="widget-content nopadding">
								<form action="{{URL::asset('navbar_saves')}}" method="post" class="form-horizontal" enctype="multipart/form-data" />
										...
										<div class="controls">
                                            <select name="pid" id="">
                                                ...
                                                @foreach($data as $key => $val)
                                                    ...
                                                    <option value="{{$val->id}}"  @if($result['pid'] == $val->id) selected="selected" @endif>{{$val->navbar_name}}</option>
                                                @foreach($val->clide as $k => $v)
                                                        <option value="{{$v->id}}" @if($result['pid'] == $val->id) selected="selected" @endif>{{$v->xian}}{{$v->navbar_name}}</option>
                                                    @if(isset($v))
                                                    @foreach($v->clide as $k => $value)
                                                        <option value="{{$value->id}}" @if($result['pid'] == $val->id) selected="selected" @endif>{{$v->xian}}{{$value->navbar_name}}</option>
                                                    @endforeach
                                                    @endif
                                                @endforeach
                                                @endforeach
                                    ...
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            <input type="text" name="navbar_name" value="{{$result['navbar_name']}}"/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            <input type="text" name="catalog" value="{{$result['catalog']}}"/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            {{--<textarea name="content" id="" cols="30" rows="10">{{$result['content']}}</textarea>--}}
                                            <textarea name="content" class="common-textarea" id="textarea" cols="30" style="width: 98%;" rows="10">{{$result['content']}}</textarea>
                                        </div>
                                    </div>
