					...
					<div class="span12">
						<div class="widget-box">
							...
							<div class="widget-content nopadding">
								<form id="form-wizard" class="form-horizontal" action="{{URL::asset('role_saves')}}" method="post" />
									<div id="form-wizard-1" class="step">
										<div class="control-group">
											<label class="control-label">角色名称</label>
											<div class="controls">
												<input id="username" type="text" name="role_name" value="{{$data['role_name']}}"  />
											</div>
                                            ...
                                            <div class="controls">
                                                {{--@foreach($data as $key => $sort)--}}
                                                    {{--@if($sort -> parent_id == 0 )--}}
                                                        {{--<input type="checkbox" name="authz_id" value="{{ $sort -> id}}" /> {{$sort->authz_name}}--}}
                                                    {{--@else--}}
                                                        {{--<input type="checkbox" name="authz_id" value="{{$sort->id}}" /> {{$sort->authz_name}}--}}
                                                    {{--@endif--}}
                                                {{--@endforeach--}}
